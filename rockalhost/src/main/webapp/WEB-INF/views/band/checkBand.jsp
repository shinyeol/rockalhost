...
<script>
   function nickOverlapCheck(){
 	  ...
 	  var b_name_label = document.getElementById("nickName_label");
 	  var regExp_b_name = /^[\w\Wㄱ-ㅎㅏ-ㅣ가-힣]{2,20}$/;
         ...
         document.form1.action="${path}/band/popBandCheck.do";
         document.form1.submit();
      }
   function apply(){
	 
		opener.document.form1.b_name.value = document.getElementById("b_name").value;
		if (opener != null) {
	    	opener.chkForm = null;
	    	self.close();
		}	
...
<input style="width: 395px" id="b_name" name="b_name" onchange="b_nameCheck()" value="${b_name}">
<input style="width: 100px;" type="button"  onclick="nickOverlapCheck()" value="중복체크" class="btn" >
<c:if test="${message == 'possible' }">
<label style="color: blue;" id="b_name_label">사용가능한 밴드이름입니다.</label>
<button type="button" onclick="apply()" class="btn">적용하기</button>
</c:if>
<c:if test="${message == 'impossible' }">
<label style="color: red;" id="b_name_label">이미 사용중인 밴드이름입니다.</label>
</c:if>
</form> 
